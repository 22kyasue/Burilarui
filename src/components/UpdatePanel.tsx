            ...
            <>
              <div className={theme === 'dark' ? 'divide-y divide-gray-700' : 'divide-y divide-gray-100'}>
                {filteredPrompts.map((chat) => {
                  ...
                  return (
                    <div key={chat.id} className={`last:border-b-0 ${theme === 'dark' ? 'border-b border-gray-700' : 'border-b border-gray-100'
                      }`}>
                      {/* プロンプトヘッダー */}
                      <div
                        onClick={() => {
                        ...
                        className={`px-6 py-3 transition-colors cursor-pointer select-none ${theme === 'dark' ? 'hover:bg-gray-700/50' : 'hover:bg-indigo-50/30'
                          }`}
                      >
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${chat.trackingActive
                              ? 'bg-gradient-to-r from-green-400 to-emerald-500 shadow-lg shadow-green-400/50'
                              : 'bg-gradient-to-r from-red-400 to-rose-500'
                            }`} />
                          <span className={`text-sm font-semibold line-clamp-1 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-900'
                        ...
                        <div className={theme === 'dark' ? 'bg-gray-900/50' : 'bg-gray-50/50'}>
                          {promptUpdates.map((update, index) => (
                            <div
                              ...
                              onMouseDown={(e) => e.preventDefault()}
                              className={`px-6 py-3 transition-colors cursor-pointer border-t first:border-t-0 select-none ${theme === 'dark'
