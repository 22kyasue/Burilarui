...
export function TrackingPage({ onBack, promptId, isSidebarOpen, onToggleSidebar, theme = 'light', chats }: TrackingPageProps) {
  ...
  const generateBestAnswer = (prompt: TrackingPrompt) => {
    const summaries: Record<string, { summary: string; full: string }> = {
      '1': {
        summary: 'ご提示いただいた設問（「印象的な取り組み」と「それへの貢献」）に対し、リサーチ結果に基づいた3つの理想的なストーリー構成案をご提案します。これらは、単に「すごいと思いました」という感想にとどまらず、「ユニリーバの戦略（コンパス）を深く理解している」ことと、「自分のキャリアビジョン」を論理的に接続させるための構成です。',
        full: 'Netflix Warner Bros買収に関する最新の詳細が明らかになりました。この買収は、Netflixがワーナーブロスを完全に所有するための重要なステップであり、メディア業界の大きな変化をもたらす可能性があります。\n\n買収の詳細は、取引価格、競合他社からの入札状況、特定の資産の除外、そして締結予定のタイムラインが含まれています。これらの情報は、買収の成功確率と影響範囲を評価する上で重要な指標となります。',
      },
      '2': {
        ...
        full: 'React 19ベータ版では、Server Componentsの大幅な改善とReact Compilerの導入により、パフォーマンスと開発者体験が向上します。新しいAPI仕様により、従来の手動最適化が不要になり、自動的にメモ化が行われます。Suspenseの改善や新しいuseフックにより、非同期処理がより直感的に扱えるようになりました。\n\nServer Componentsは、クライアント側のJavaScriptバンドルサイズを削減しながら、サーバー側でのデータフェッチを可能にします。これにより、初期ロード時間が大幅に短縮され、ユーザー体験が向上します。',
      },
    };

    return summaries[prompt.id] || {
      summary: `${prompt.title}に関する最新のアップデート情報を含む包括的な回答です。各アップデートの内容を統合し、実用的なベストプラクティスと具体的な実装例を提供します。`,
      full: `${prompt.title}に関する最新のアップデート情報を含む包括的な回答です。各アップデートの内容を統合し、実用的なベストプラクティスと具体的な実装例を提供します。\n\n詳細な技術解説、実装パターン、パフォーマンス最適化の手法、そして実際のユースケースを含めた完全な回答がここに表示されます。`,
    };
  };
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-sm">
                                <div className="text-gray-900 leading-relaxed text-sm">
                                  {generateAnswerWithHighlights(expandedPrompt).map((segment, idx) => {
                                    ...
                                    return (
                                      ...
                                      >
                                        <span className="bg-yellow-500/20 border-b-2 border-yellow-500/50 rounded px-1 whitespace-pre-line inline-block relative">
                                          {segment.text}
                                        ...
                                        {hoveredSegment === segmentId && segment.updateInfo && (
                                          <div className="absolute bottom-full left-0 mb-2 z-10 w-72 bg-white border border-amber-300 rounded-xl shadow-xl p-3 animate-in fade-in duration-200">
                                            <div className="flex items-start gap-2">
                                              ...
                                              <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                  <span className="text-xs text-amber-700 font-semibold">最新アップデート</span>
                                                  <span className="text-xs text-gray-600">{formatDateTime(segment.updateInfo.timestamp)}</span>
                                                </div>
